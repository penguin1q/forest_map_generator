...
- [Key Components](#key-components)
  ...
  - [7. convert_tree_asset_to_gazebo workflow](#7-convert_tree_asset_to_gazebo-workflow)
...
2) tree assets (built-in models, point-cloud-derived meshes, or private high-quality Blender assets converted into Gazebo models).
...
5) **Create Gazebo-ready tree models from point clouds or private Blender assets (optional)**
   ...
   - `scripts/convert_tree_asset_to_gazebo/convert_tree_asset_to_gazebo.py` converts prepared private Blender tree assets into Gazebo model directories, typically under `models_private/<model_name>/`, including:
     - `meshes/tree_mesh.obj` (visual mesh)
     - `meshes/tree_collision.stl` (collision mesh)
...
│       ├── convert_tree_asset_to_gazebo.py  # Prepared Blender asset → Gazebo model
│       └── README.md
│
├── models/
...
---
### 7. convert_tree_asset_to_gazebo workflow

**Location**
```text
scripts/convert_tree_asset_to_gazebo/
├── convert_tree_asset_to_gazebo.py
└── README.md
```

**Role**
This workflow converts a prepared private Blender tree asset into a Gazebo model directory. It is intended for high-quality mikan/orange tree assets created locally with manual modeling, add-ons, purchased models, or scans. The repository contains only the converter and documentation; private `.blend` files, purchased assets, large meshes, and textures should stay outside Git.

**Recommended preparation**
1) Create or edit the tree locally in Blender.
2) If using add-ons, Geometry Nodes, particles, or instancing, convert/apply/realize procedural content to mesh when needed.
3) Use meters as the unit scale.
4) Place the trunk base / ground contact point near `z=0`.
5) Optionally create a `visual` collection for visual geometry.
6) Optionally create a `collision` collection for custom simplified collision geometry.
...
The default `--origin-mode bottom_center` shifts the visual bounds so the bottom becomes `z=0` and the XY center becomes `0,0`. Other modes are `keep`, `cursor`, and `named_empty` (`TREE_ORIGIN` by default). Blender and Gazebo are both treated as Z-up systems: X/Y are horizontal and Z is up. OBJ export requests `forward_axis=Y, up_axis=Z` when the installed Blender supports those options.

The converter prints visual bounds before export. If the model appears sideways, check the input Blender model rotation, the printed OBJ axis settings, and that `model.sdf` does not contain a hidden visual pose rotation.

**Collision**
The converter always writes `meshes/tree_collision.stl`. Collision priority is `--collision-file`, `collision` collection, `thick_branch_cylinders`, `trunk_cylinder`, then other generated modes. The `thick_branch_cylinders` mode creates coarse cylinders from Sapling/Curve-derived thick trunk and main branch segments, and does not add the fixed upright trunk cylinder by default. It filters out leaves, fruits, thin twigs, and upper crown branches by name/radius/height. Use `--collision-add-trunk-cylinder` only when an additional simple trunk cylinder is desired. `bounding_box`, `bounding_cylinder`, and `visual_decimated_optional` are also available, but full visual mesh collision is not recommended. For best control, create a Blender collection named `collision` containing simplified trunk/main-branch geometry.

**Private model resource path**
When outputting to `models_private/<model_name>/` outside the package, add the parent directory to the Gazebo resource path:

```bash
export IGN_GAZEBO_RESOURCE_PATH=$IGN_GAZEBO_RESOURCE_PATH:/path/to/models_private
```

For newer GZ Sim environments:

```bash
export GZ_SIM_RESOURCE_PATH=$GZ_SIM_RESOURCE_PATH:/path/to/models_private
```

Use the generated model in QGIS / GeoJSON placement:

```text
tree_type = orange_tree_mikan_01
```

or comma-separated variants:

```text
tree_type = orange_tree_mikan_01,orange_tree_mikan_02,orange_tree_mikan_03
```

Blender is needed only while converting assets. Gazebo runtime does not need Blender once the generated model directories are available through the resource path.
