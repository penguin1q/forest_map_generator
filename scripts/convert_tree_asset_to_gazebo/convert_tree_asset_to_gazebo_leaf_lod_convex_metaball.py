...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Convert a Blender tree asset into a Gazebo model directory with optional convex-hull or metaball leaf LOD."
    )
    ...
    parser.add_argument(
        "--leaf-lod-mode",
        choices=("original", "joined", "convex_hull", "metaball", "none"),
        default="original",
        help="How to export leaf visuals. convex_hull / metaball replace leaf meshes with clustered low-poly leaf clumps.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Maximum vertices sampled from each leaf object for hull / metaball source point construction.",
    )

    parser.add_argument(
        "--leaf-metaball-radius",
        type=float,
        default=0.22,
        help="Radius of each metaball element used for leaf clump generation [m].",
    )
    parser.add_argument(
        "--leaf-metaball-resolution",
        type=float,
        default=0.18,
        help="Metaball viewport resolution. Larger values create coarser, lighter meshes.",
    )
    parser.add_argument(
        "--leaf-metaball-render-resolution",
        type=float,
        default=None,
        help="Metaball render resolution. Defaults to --leaf-metaball-resolution.",
    )
    parser.add_argument(
        "--leaf-metaball-threshold",
        type=float,
        default=0.6,
        help="Metaball threshold. Higher values separate clumps more; lower values merge them more.",
    )
    parser.add_argument(
        "--leaf-metaball-decimate-ratio",
        type=float,
        default=0.35,
        help="Decimate ratio applied after converting metaballs to mesh.",
    )
    parser.add_argument(
        "--leaf-metaball-point-stride",
        type=int,
        default=8,
        help="Use one source point every N points within each cluster for metaball generation.",
    )
    parser.add_argument(
        "--leaf-metaball-max-points",
        type=int,
        default=900,
        help="Maximum total leaf points sampled before clustering for metaball LOD.",
    )
    parser.add_argument(
        "--leaf-metaball-max-points-per-cluster",
        type=int,
        default=90,
        help="Maximum metaball source points used in one cluster after stride sampling.",
    )
    parser.add_argument(
        "--leaf-metaball-shade-smooth",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Apply smooth shading to generated metaball leaf meshes.",
    )
...
def export_obj_compatible(bpy, objects, path):
    ...
    # Preserve OBJ/MTL material information when the Blender exporter supports it.
    # This lets Gazebo / Blender re-import use the color written in the .mtl file
    # instead of requiring a forced SDF <material> override.
    if "export_materials" in properties:
        kwargs["export_materials"] = True

    if "path_mode" in properties:
...
def material_base_color_rgba(source_material, fallback=(0.0, 0.32, 0.0, 1.0)):
    ...
    # Prefer Principled BSDF Base Color when a node material is used.
...
def create_leaf_convex_hull_lod(bpy, mathutils, leaf_objects, args):
    ...
    # Generated convex hull objects do not have UVs, so copying a full texture material is
    # usually not useful. Instead, inherit the source leaf material's base color and create
    # a simple LOD material that exports cleanly to OBJ/MTL.
    source_leaf_material = first_material_from_objects(leaf_objects)
    leaf_lod_material = create_plain_material_from_source(
        bpy,
        source_leaf_material,
        "leaf_lod_from_source",
        (0.0, 0.32, 0.0, 1.0),
    )
    ...
    for idx, cluster_points in enumerate(clusters):
        ...
        if hull is not None:
            hull.data.materials.append(leaf_lod_material)
            hull_objects.append(hull)

    color = material_base_color_rgba(source_leaf_material, (0.0, 0.32, 0.0, 1.0))
    info(
    ...
    info(
        "Leaf LOD material inherited base color: "
...
def create_metaball_leaf_clump_from_points(
    bpy,
    mathutils,
    points,
    name,
    ...
    stride = max(1, args.leaf_metaball_point_stride)
    sampled = points[::stride]

    max_points = max(1, args.leaf_metaball_max_points_per_cluster)
    ...
    resolution = float(args.leaf_metaball_resolution)
    render_resolution = (
        float(args.leaf_metaball_render_resolution)
        if args.leaf_metaball_render_resolution is not None
        else resolution
    )

    try:
        bpy.ops.object.metaball_add(type="BALL", location=sampled[0])
    except Exception as exc:
        warn(f"Metaball creation failed for {name}: {exc}")
        return None

    meta_obj = bpy.context.object
    meta_obj.name = name
    meta_obj.data.name = name + "_meta"
    meta_obj.data.resolution = resolution
    meta_obj.data.render_resolution = render_resolution
    meta_obj.data.threshold = float(args.leaf_metaball_threshold)

    meta_obj.data.elements[0].radius = float(args.leaf_metaball_radius)

    for p in sampled[1:]:
        elem = meta_obj.data.elements.new(type="BALL")
        elem.co = p - meta_obj.location
        elem.radius = float(args.leaf_metaball_radius)
    ...
    try:
        ...
        warn(f"Metaball conversion failed for {name}: {exc}")
        return None

    mesh_obj = bpy.context.object
    mesh_obj.name = name + "_mesh"
    mesh_obj.data.name = name + "_mesh_data"

    if material is not None:
        mesh_obj.data.materials.append(material)

    for poly in mesh_obj.data.polygons:
        poly.use_smooth = bool(args.leaf_metaball_shade_smooth)

    if args.leaf_metaball_decimate_ratio < 0.999:
        try:
            bpy.ops.object.select_all(action="DESELECT")
            mesh_obj.select_set(True)
            bpy.context.view_layer.objects.active = mesh_obj
            modifier = mesh_obj.modifiers.new("leaf_metaball_decimate", "DECIMATE")
            modifier.ratio = max(0.05, min(1.0, args.leaf_metaball_decimate_ratio))
            bpy.ops.object.modifier_apply(modifier=modifier.name)
        except Exception as exc:
            warn(f"Could not decimate metaball mesh {name}: {exc}")

    return mesh_obj


def create_leaf_metaball_lod(bpy, mathutils, leaf_objects, args):
    if not leaf_objects:
        return []
    ...
    leaf_points = sample_all_leaf_vertices_world(
        ...
        max_total_points=max(
            300,
            args.leaf_metaball_max_points,
            args.leaf_cluster_count * args.leaf_sample_vertices_per_object * 10,
        ),
    )

    if not leaf_points:
        warn("No leaf vertices were collected for metaball LOD.")
        return []
    ...
    for idx, cluster_points in enumerate(clusters):
        obj = create_metaball_leaf_clump_from_points(
            bpy,
            mathutils,
            cluster_points,
            f"leaf_metaball_{idx:02d}",
            leaf_lod_material,
            args,
        )
        ...
        if obj is not None and obj.type == "MESH" and len(obj.data.polygons) > 0:
            ...
            warn(f"Metaball leaf clump {idx:02d} was empty or invalid.")

    color = material_base_color_rgba(source_leaf_material, (0.0, 0.32, 0.0, 1.0))
    info(
        f"Generated {len(objects)} metaball leaf LOD objects "
        f"from {len(leaf_objects)} leaf objects and {len(leaf_points)} sampled leaf points."
    )
    info(
        "Leaf metaball LOD material inherited base color: "
        f"rgba=({color[0]:.3f}, {color[1]:.3f}, {color[2]:.3f}, {color[3]:.3f})"
    )
...
def make_leaf_export_objects(bpy, mathutils, leaf_objects, args):
    ...
    if args.leaf_lod_mode == "metaball":
        return create_leaf_metaball_lod(bpy, mathutils, leaf_objects, args)
...
def validate_args(args):
    ...
    if args.leaf_metaball_radius <= 0.0:
        raise RuntimeError("--leaf-metaball-radius must be greater than zero")

    if args.leaf_metaball_resolution <= 0.0:
        raise RuntimeError("--leaf-metaball-resolution must be greater than zero")

    if args.leaf_metaball_render_resolution is not None and args.leaf_metaball_render_resolution <= 0.0:
        raise RuntimeError("--leaf-metaball-render-resolution must be greater than zero")

    if args.leaf_metaball_threshold <= 0.0:
        raise RuntimeError("--leaf-metaball-threshold must be greater than zero")

    if not (0.0 < args.leaf_metaball_decimate_ratio <= 1.0):
        raise RuntimeError("--leaf-metaball-decimate-ratio must be in (0, 1].")

    if args.leaf_metaball_point_stride <= 0:
        raise RuntimeError("--leaf-metaball-point-stride must be greater than zero")

    if args.leaf_metaball_max_points <= 0:
        raise RuntimeError("--leaf-metaball-max-points must be greater than zero")

    if args.leaf_metaball_max_points_per_cluster <= 0:
        raise RuntimeError("--leaf-metaball-max-points-per-cluster must be greater than zero")
...
def convert(args):
    ...
    if split_visual_groups is not None:
        ...
        # tree_mesh.objも互換用に出す．LOD leaf + woodの合成にする．
        combined_export_objects = list(wood_objects) + list(leaf_export_objects)
